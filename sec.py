...
class SECSession:
    ...
    def get_filing(self, filing_url):
        return self.session.get(filing_url, headers=self.headers)

    @staticmethod
    def _validate_payload(**kwargs):
        valid_kwargs = {"category", "startdt", "enddt", "forms", "dateRange"}
        invalid_fields = set(kwargs) - valid_kwargs
        if invalid_fields:
            raise RuntimeError(f"Invalid payload fields: {', '.join(invalid_fields)}")
...
if __name__ == '__main__':
    ...
    print(sec.get_filing_url(company_name="Apple Inc", company_ticker="AAPL"))
